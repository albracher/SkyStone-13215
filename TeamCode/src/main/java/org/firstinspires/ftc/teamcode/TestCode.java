...
*/
    ...
    public void strafe(double speed, int distance) {
        ...
        robot.motorFL.setPower(Math.abs(speed));
        // Motors are set to run at a certain speed until one reaches its target position
        while (robot.motorFL.isBusy()) {
            telemetry.addData("Motor FL", speed);
            telemetry.addData("fetchedSpeed", fetchedSpeed);
            telemetry.update();
            if(distance>0) {
                fetchedSpeed = robot.motorFL.getPower();
            }else{
                fetchedSpeed = -robot.motorFL.getPower();
            }

            robot.motorFR.setPower(-fetchedSpeed);
            robot.motorRL.setPower(-fetchedSpeed);
            robot.motorRR.setPower(fetchedSpeed);
